...
screen_size_display = (width_screen, height_screen) = (600, 200)
FPS = 60
gravity = 0.6
...
pygame.display.set_caption("Real-life Disconnected ")
...
class Girl():
    def __init__(self, sx=-1, sy=-1):
        self.imgs, self.rect = load_sprite_sheet('girl.png', 5, 1, sx, sy, -1)
        self.imgs1, self.rect1 = load_sprite_sheet('girl_ducking.png', 2, 1, 59, sy, -1)
...
class Socialapp(pygame.sprite.Sprite):
    def __init__(self, speed=5, sx=-1, sy=-1):
        pygame.sprite.Sprite.__init__(self,self.containers)
        self.imgs, self.rect = load_sprite_sheet('socialapp.png', 3, 1, sx, sy, -1)
        self.rect.bottom = int(0.98 * height_screen)
...
def introduction_screen():
    ...
    while not starting_game:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_SPACE or event.key == pygame.K_UP:
                        ado_girl.jumping = True
                        ado_girl.blinking = False
                        ado_girl.movement[1] = -1*ado_girl.jumpSpeed
        ...
        time_clock.tick(FPS)
        if ado_girl.jumping == False and ado_girl.blinking == False:
            starting_game = True
    ...
    while not g_exit:
        ...
        while not g_Over:
                ...
                for event in pygame.event.get():
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            if gamer_Girl.rect.bottom == int(0.98 * height_screen):
                                gamer_Girl.jumping = True
                                if pygame.mixer.get_init() != None:
                                    jump_sound.play()
                                gamer_Girl.movement[1] = -1*gamer_Girl.jumpSpeed

                        if event.key == pygame.K_DOWN:
                            if not (gamer_Girl.jumping and gamer_Girl.dead):
                                gamer_Girl.ducking = True

                    if event.type == pygame.KEYUP:
                        if event.key == pygame.K_DOWN:
                            gamer_Girl.ducking = False
            for c in socialapps:
                c.movement[0] = -1*gp
                if pygame.sprite.collide_mask(gamer_Girl,c):
                    gamer_Girl.dead = True
                    if pygame.mixer.get_init() != None:
                        die_sound.play()

            for p in smallBird:
                p.movement[0] = -1*gp
                if pygame.sprite.collide_mask(gamer_Girl,p):
                    gamer_Girl.dead = True
                    if pygame.mixer.get_init() != None:
                        die_sound.play()
            ...
            if len(socialapps) < 2:
                if len(socialapps) == 0:
                    last_end_obs.empty()
                    last_end_obs.add(Socialapp(gp,40,40))
                else:
            ...
            skyClouds.update()
            new_grnd.update()
            score_boards.update(gamer_Girl.score)
            ...
            if pygame.display.get_surface() != None:
                ...
                socialapps.draw(screen_layout_display)
            ...
            if gamer_Girl.dead:
                g_Over = True
                if gamer_Girl.score > highest_scores:
                    highest_scores = gamer_Girl.score
